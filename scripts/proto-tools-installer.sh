...
f_ensure_tools() {
    ! which curl &>/dev/null && f_abort 2 "couldn't find curl, aborting"
}
...
f_install_protoc_gen_gocosmos() {
    ...
    if ! grep "github.com/gogo/protobuf => github.com/regen-network/protobuf" go.mod &>/dev/null; then
        echo -e "\tPlease run this command from somewhere inside the DSC folder."
        return 1
    fi
...
f_install_clang_format() {
    ...
    Linux)
        ...
        ;;
    Darwin)
        echo -e "\tRun: brew install clang-format" >&2
        ;;
    *)
