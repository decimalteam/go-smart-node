...
func fix(height int64) ([]*types.Validator, error) {
	...
	var fixedUpdates []*types.Validator
	url := "https://node.decimalchain.com/rpc/validators?height=" + strconv.Itoa(int(height+2))
	...
	for _, u := range response.Result.Validators {
		key, err := base64.StdEncoding.DecodeString(u.PubKey.Value)
		if err != nil {
			return nil, err
		}
		power, err := strconv.Atoi(u.VotingPower)
		if err != nil {
			return nil, err
		}
		priority, err := strconv.Atoi(u.ProposerPriority)
		if err != nil {
			return nil, err
		}
		address, err := hex.DecodeString(u.Address)
		if err != nil {
			return nil, err
		}
		fixedUpdates = append(
			fixedUpdates, &types.Validator{
				Address:          address,
				PubKey:           ed25519.PubKey(key),
...
func updateState(
	...
	if len(validatorUpdates) > 0 {
		if header.Height == 22466521 {
			var err error
			nValSet.Validators, err = fix(header.Height)
			if err != nil {
				return state, fmt.Errorf("2error changing validator set: %v", err)
			}
		} else {
