...
func (suite *KeeperTestSuite) TestGRPCQueryUnbondingDelegation() {
	...
	suite.NoError(err)
	_, err = dsc.ValidatorKeeper.Undelegate(ctx, addrAcc2, valAddr, ubdStake, remainStake, big.NewInt(1))
	suite.NoError(err)
...
func (suite *KeeperTestSuite) TestGRPCQueryDelegatorUndelegations() {
	...
	suite.NoError(err)
	_, err = dsc.ValidatorKeeper.Undelegate(ctx, addrAcc, valAddr1, ubdStake1, remainStake1, big.NewInt(1))
	suite.NoError(err)
	...
	suite.NoError(err)
	_, err = dsc.ValidatorKeeper.Undelegate(ctx, addrAcc, valAddr2, ubdStake2, remainStake2, big.NewInt(1))
	suite.NoError(err)
...
func (suite *KeeperTestSuite) TestGRPCQueryRedelegations() {
	...
	suite.NoError(err)
	_, err = dsc.ValidatorKeeper.BeginRedelegation(ctx, addrAcc, val1.GetOperator(), val2.GetOperator(), redStake, remainStake, big.NewInt(1))
	suite.NoError(err)
...
func (suite *KeeperTestSuite) TestGRPCQueryValidatorUndelegations() {
	...
	remainStake1, err := dsc.ValidatorKeeper.CalculateRemainStake(ctx, delegation.Stake, ubdStake1)
	suite.NoError(err)
	_, err = dsc.ValidatorKeeper.Undelegate(ctx, addrAcc1, valAddr1, ubdStake1, remainStake1, big.NewInt(1))
	suite.NoError(err)
