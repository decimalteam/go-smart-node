...
func TestUpdateCoin(t *testing.T) {
	...
	require.NoError(t, err)
	err = processEventUpdateCoin(ea, abci.Event(ev), "ABCD", 7)
	require.NoError(t, err)
	ev, err = utilsEvents.TypedEventToEvent(&coinTypes.EventUpdateCoin{
		Sender:      "a",
		Denom:       "test",
...
func TestEditCoin(t *testing.T) {
	...
	require.NoError(t, err)
	err = processEventUpdateCoinVR(ea, abci.Event(ev), "ABCD", 7)
	require.NoError(t, err)
	ev, err = utilsEvents.TypedEventToEvent(&coinTypes.EventUpdateCoinVR{
		Denom:   "test",
		Volume:  "1001",
...
func TestSendCoin(t *testing.T) {
	...
	require.NoError(t, err)
	err = processEventSendCoin(ea, abci.Event(ev), "ABCD", 7)
	require.NoError(t, err)
	ev, err = utilsEvents.TypedEventToEvent(&coinTypes.EventSendCoin{
		Sender:    "b",
		Recipient: "a",
...
func TestBuySellCoin(t *testing.T) {
	...
	require.NoError(t, err)
	err = processEventBuySellCoin(ea, abci.Event(ev), "ABCD", 7)
	require.NoError(t, err)
...
func TestBurnCoin(t *testing.T) {
	...
	require.NoError(t, err)
	err = processEventBurnCoin(ea, abci.Event(ev), "ABCD", 7)
	require.NoError(t, err)
...
func TestMultisigComplete(t *testing.T) {
	...
	ev, err := utilsEvents.TypedEventToEvent(tev)
	require.NoError(t, err)
	err = processEventConfirmTransaction(ea, abci.Event(ev), "", 0)
	require.NoError(t, err)
...
func TestMultisigCreateTx(t *testing.T) {
	...
	ev, err := utilsEvents.TypedEventToEvent(tev)
	require.NoError(t, err)
	err = processEventCreateTransaction(ea, abci.Event(ev), "", 0)
	require.NoError(t, err)
	...
	require.Equal(t, "tx", ea.MultisigCreateTxs[0].TxHash)
...
func TestMultisigSignTx(t *testing.T) {
	...
	ev, err := utilsEvents.TypedEventToEvent(tev)
	require.NoError(t, err)
	err = processEventSignTransaction(ea, abci.Event(ev), "", 0)
	require.NoError(t, err)
	...
	require.Equal(t, "tx", ea.MultisigSignTxs[0].TxHash)
